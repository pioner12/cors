const APP_NAME = "golgetv"
const HOSTNAME = "golgetv"
const APP_KEY = ""
...
async function handleRequest(request) {
  let url = new URL(request.url)
  let apiurl = request.url.replace(`${url.origin}/`, '');
  // Rewrite request to point to API url. This also makes the request mutable
  // so we can add the correct Origin header to make the API server think
  // that this request isn't cross-site.
  if (apiurl === "") {
    return new Response('Yarrak kafalı japon askeri senii...GolgeTV Adamı siker')
  }
...
const postLog = request => {
  ...
  try {
    return fetch(
      "https://logs.logdna.commmmm/logs/ingest?tag=worker&hostname=" + HOSTNAME,
      {
        method: "POST",
        headers: myHeaders,
        body: data
